(function (window) {
  ...
  p.onKeyUp = function (e) {
    ...
    if (c == "R") this.reset();
      ...
      this.debugNoFill = !this.debugNoFill;
      this.draw();
    }
  };
  ...
  p.setSpeed = function (value) {
    this.speed = value;
    if (Math.abs(value) < 0.001) this.speed = 0;
  };
  ...
  p.drawRoll = function (radius, trail, remainder) {
    var m = Math,
      ...
      PI5 = PI * 0.5;
    var cfg = this.config,
      px = cfg.perspectiveOffsetX,
      py = cfg.perspectiveOffsetY,
      f = cfg.perspectiveFactor;
    var lw = this.dpr;
    ...
    if (arc > aBtm) {
      var tx = radius * m.cos(arc),
        ty = radius * m.sin(arc);
      if (fill) g.f(cfg.colorUnder);
      g.s(cfg.colorUnder).ss(lw).mt(pBtm.x, pBtm.y).lt(pBtm2.x, pBtm2.y).arc(px, py, radius2, aBtm, arc, false).lt(tx, ty).arc(0, 0, radius, arc, aBtm, true).ef().es();
    }
    ...
    if (remainder >= cfg.perimeterMin) {
      ...
      if (a > aTop) {
        g.ef().s(cfg.colorSide).ss(lw).arc(0, 0, radius, aTop, a).es();
      }
    }
    //Draw trail
    ...
    if (trail < cfg.rollLength + maxStraight) {
      ...
      for (var i = 0; i < n; i++) {
        var len = i * cfg.sheetLength,
          y = y0 + len;
        var p = this.projectXY(-radius, y);
        g.mt(-radius, y).lt(p.x, p.y);
      }
      //	curved part top
      if (remainder > cfg.sheetLength) {
        // for no dashed line on last sheet
        //if (remainder>0)
        for (var i = 1; i < (rest + curveTopLen) / cfg.sheetLength; i++) {
          var ang = m.PI + (i * cfg.sheetLength - rest) / radius;
          g.mt(radius * m.cos(ang), radius * m.sin(ang)).lt(px + radius2 * m.cos(ang), py + radius2 * m.sin(ang));
        }
      }
    }
    //
